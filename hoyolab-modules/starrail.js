...
module.exports = class StarRail extends require("./template.js") {
	...
	async login () {
		...
		for (const account of accounts) {
			...
			const isV2 = (ltoken.startsWith("v2_"));
			const cookieData = `ltuid${isV2 ? "_v2" : ""}=${ltuid}; ltoken${isV2 ? "_v2" : ""}=${ltoken};`;
	...
	async checkAndExecute () {
		...
		for (const account of accounts) {
			...
			if (data.isSigned) {
				app.Logger.info("StarRail:CheckIn", `${account.nickname} already signed in today`);
				continue;
			}
